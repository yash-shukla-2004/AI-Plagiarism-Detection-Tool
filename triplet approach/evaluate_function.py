...
def get_plagiarism_percentage(model, train_loader, suspect_data, margin=1.0):
    """
    ...
    with torch.no_grad():
        ...
        for anchor_data,_,_ in train_loader:
            anchor_emb = model(anchor_data.x, anchor_data.edge_index, anchor_data.batch) #error line
        suspect_emb = model(suspect_data.x, suspect_data.edge_index, suspect_data.batch)
        #print(suspect_emb)
        for module in model.modules():
            if isinstance(module, torch.nn.Dropout):
                print("Dropout active:", module.training)  # Should be False
...
def load_model(model, file_path):
    ...
    # Should match before and after saving
    return model

# Example usage:
# Assuming you have the model class defined already, e.g., PlagiarismDetector
model = GNNModel()  # Create a new instance of the model
model = load_model(model, 'plagiarism_detector_model_1.pth')
model.eval()
